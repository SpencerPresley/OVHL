    ...
    for (const player of playersInBidding) {
      try {
        ...
        if (player.teamSeasons && player.teamSeasons.length > 0) {
          console.log(
            `Player ${player.player.name} (ID: ${player.id}) is already on a team but marked as in bidding. Fixing...`
          );
          alreadyOnTeamCount++;
        ...
        if (!redisData) {
          console.log(
            `Player ${player.player.name} (ID: ${player.id}) is marked as in bidding but not in Redis. Fixing...`
          );
          notInRedisCount++;
          ...
          console.log(
            `Player ${player.player.name} (ID: ${player.id}) is marked as completed in Redis but still in bidding in DB. Fixing...`
          );
