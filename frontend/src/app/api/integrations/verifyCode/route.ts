...
export async function POST(request: Request) {
  try {
      ...
      try {
        ...
        if (profileData.about_me && profileData.about_me.includes(code)) {
          ...
          return NextResponse.json(
            {
              error:
                'Verification code not found in your PSN profile. Please make sure you have added the code to your About Me section and try again.',
            },
            { status: 400 }
          );
        }
        ...
        return NextResponse.json(
          {
            error:
              'Failed to verify profile. The PSN API may be temporarily unavailable or the profile may not exist.',
          },
          { status: 500 }
        );
      }
    ...
    return NextResponse.json({ error: 'Failed to verify integration' }, { status: 500 });
  }
}
...
async function savePSNProfileData(userId: string, onlineId: string, profileData: any) {
  try {
    ...
    if (profileData.avatars && profileData.avatars.length > 0) {
      ...
      const largestAvatar =
        profileData.avatars.find((a: any) => a.size === 'xl') ||
    ...
    if (
      profileData.trophy_level ||
      (profileData.earned_trophies && Object.keys(profileData.earned_trophies).length > 0)
    ) {
      await prisma.pSNTrophy.upsert({
        ...
        create: {
          ...
          totalTrophies:
            (profileData.earned_trophies?.platinum || 0) +
