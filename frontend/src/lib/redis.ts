...
export const biddingUtils = {
  ...
  async placeBid(
    ...
    const wasOutbid =
      playerData.currentTeamId !== null && playerData.currentTeamId !== bidData.teamId;
  ...
  async getTeamCommittedBids(teamId: string) {
    ...
    for (const key of keys) {
      ...
      if (data) {
        try {
          ...
          console.log(
            `Player ${player.playerName} (${playerIdFromKey}): currentTeamId=${player.currentTeamId}, status=${player.status}, currentBid=${player.currentBid}`
          );
    ...
    console.log(
      `Team ${teamId} total committed: ${totalCommitted}, active bids: ${activeBids.length}`
    );
