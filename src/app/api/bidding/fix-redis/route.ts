...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    for (const key of allKeys) {
      ...
      if (isOnTeam) {
        console.log(`Player ${playerInDB.player.name} (${playerSeasonId}) is on a team but still in Redis`);
      ...
      if (!playerInDB.isInBidding && isActive) {
        console.log(`Player ${playerInDB.player.name} (${playerSeasonId}) is not in bidding in DB but is active in Redis`);
    ...
    return NextResponse.json(
      { error: 'Failed to fix Redis data' }, 
      { status: 500 }
    );
  }
} 