...
describe('Standings API Route', () => {
  ...
  describe('Team Stats and Division Grouping', () => {
    it('correctly calculates team stats and groups by division', async () => {
      ...
      const atlanticDivision = data.standings.find(
        (d: DivisionStandings) => d.division === 'Atlantic'
      );
      ...
      const torontoTeam = atlanticDivision?.teams.find(
        (t: TeamStats) => t.teamIdentifier === 'TOR'
      );
    ...
    it('ignores teams with unknown divisions', async () => {
      ...
      mockFindFirst.mockResolvedValueOnce(mockSeason).mockResolvedValueOnce(mockTier);
