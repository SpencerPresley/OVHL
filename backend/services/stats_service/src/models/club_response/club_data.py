...
class ClubData(BaseModel):
    ...
    div_groups_won: int = Field(
        alias="divGroupsWon", description="Total division groups won"
    )
    leagues_won: int = Field(alias="leaguesWon", description="Total leagues won")

    # Division Group Wins by Level
    div_groups_won1: int = Field(
        alias="divGroupsWon1", description="Division 1 groups won"
    )
    div_groups_won2: int = Field(
        alias="divGroupsWon2", description="Division 2 groups won"
    )
    div_groups_won3: int = Field(
        alias="divGroupsWon3", description="Division 3 groups won"
    )
    div_groups_won4: int = Field(
        alias="divGroupsWon4", description="Division 4 groups won"
    )
    ...
    ranking_points: int = Field(
        alias="rankingPoints", description="Current ranking points"
    )

    # Division Information
    cur_competition: int = Field(
        alias="curCompetition", description="Current competition ID"
    )
    prev_division: int = Field(alias="prevDivision", description="Previous division")
    prev_game_division: int = Field(
        alias="prevGameDivision", description="Previous game division"
    )
    best_division: int = Field(
        alias="bestDivision", description="Best division achieved"
    )
    best_points: int = Field(alias="bestPoints", description="Best points achieved")
    cur_season_mov: int = Field(
        alias="curSeasonMov", description="Current season movement"
    )
    ...
    prev_season_wins: int = Field(
        alias="prevSeasonWins", description="Previous season wins"
    )
    prev_season_losses: int = Field(
        alias="prevSeasonLosses", description="Previous season losses"
    )
    prev_season_ties: int = Field(
        alias="prevSeasonTies", description="Previous season ties"
    )
    prev_season_otl: int = Field(
        alias="prevSeasonOtl", description="Previous season overtime losses"
    )
    ...
    cup_win_percent: int = Field(
        alias="cupWinPercent", description="Cup win percentage"
    )
    titles_won: int = Field(alias="titlesWon", description="Total titles won")
    prev_game_won_title: int = Field(
        alias="prevGameWonTitle", description="Previous game title win flag"
    )
    ...
    # Current Status
    current_division: int = Field(
        alias="currentDivision", description="Current division"
    )
    club_info: ClubInfo = Field(
        alias="clubInfo", description="Detailed club information"
    )
