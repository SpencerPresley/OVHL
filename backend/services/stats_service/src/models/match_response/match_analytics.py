...
class MatchAnalytics:
    ...
    def get_possession_metrics(self) -> Optional[PossessionMetrics]:
        ...
        return PossessionMetrics(
            possession_differential=float(
                home_agg.skpossession - away_agg.skpossession
            ),
            possession_percentage_home=(
                (float(home_agg.skpossession) / total_possession * 100)
                if total_possession > 0
                else 50.0
            ),
            possession_percentage_away=(
                (float(away_agg.skpossession) / total_possession * 100)
                if total_possession > 0
                else 50.0
            ),
            time_on_attack_differential=float(home_club.time_on_attack)
            - float(away_club.time_on_attack),
        )
    ...
    def get_efficiency_metrics(self) -> Optional[EfficiencyMetrics]:
        ...
        return EfficiencyMetrics(
            home_shooting_efficiency=(
                (float(home_club.goals) / float(home_club.shots) * 100)
                if float(home_club.shots) > 0
                else 0.0
            ),
            away_shooting_efficiency=(
                (float(away_club.goals) / float(away_club.shots) * 100)
                if float(away_club.shots) > 0
                else 0.0
            ),
            home_passing_efficiency=(
                (
                    float(home_club.passes_completed)
                    / float(home_club.passes_attempted)
                    * 100
                )
                if float(home_club.passes_attempted) > 0
                else 0.0
            ),
            away_passing_efficiency=(
                (
                    float(away_club.passes_completed)
                    / float(away_club.passes_attempted)
                    * 100
                )
                if float(away_club.passes_attempted) > 0
                else 0.0
            ),
            home_possession_efficiency=(
                float(home_club.time_on_attack) / 3600 * 100
            ),  # Convert to percentage of game
    ...
    def get_special_teams_metrics(self) -> Optional[SpecialTeamsMetrics]:
        ...
        return SpecialTeamsMetrics(
            home_powerplay_pct=(
                (
                    float(home_club.powerplay_goals)
                    / float(home_club.powerplay_opportunities)
                    * 100
                )
                if float(home_club.powerplay_opportunities) > 0
                else 0.0
            ),
            away_powerplay_pct=(
                (
                    float(away_club.powerplay_goals)
                    / float(away_club.powerplay_opportunities)
                    * 100
                )
                if float(away_club.powerplay_opportunities) > 0
                else 0.0
            ),
